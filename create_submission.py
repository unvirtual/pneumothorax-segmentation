...
if __name__ == "__main__":
    ...
    cp = CheckPoint.load("eval/UResNet++34D_512x512_SGD_ROP_NoInterpolation_r1/last_checkpoint.pth", model, device=DEV)
    results = predict_rles(cp.model, "resnet34", "imagenet", "data-original/dicom-images-test", 0.9, 50, 512)
    pd.DataFrame(results, columns=["ImageId","EncodedPixels"]).to_csv("submission_uresnet34++_512.csv", index=False)
